...
`conan` too. So it should be fairly obvious how to use it if you already know
...
But to make things even easier there is a `barbarian new` tool to help you
easily create simple package recipes to get you started.
...
in a `<git-repo-root>/.conan` directory. Following with the previous
`test` recipe:
...
TIP: It's recommended to add `/.conan/` to your `.gitignore` root file to
avoid accidentally adding this internal directory to git.
